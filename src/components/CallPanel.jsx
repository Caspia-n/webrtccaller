...
export default function CallPanel(){
  ...
  function log(...args){
    setLogs(prev => {
      const entry = args.map(a => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')
  ...
  async function ensurePeer(){
    ...
    }catch(e){ log('getUserMedia fail', e) }
  }
  ...
  async function startCall(){
    ...
    }catch(e){ log('DC create failed', e) }
    ...
    <div className="call-panel">
      <div className="video-row">
        <div className="video-card glass">
          <video ref={localVideoRef} playsInline autoPlay muted className="local" />
      ...
      <div className="controls">
        {!connected && <button className="btn" onClick={connectWS}>Connect Signaling</button>}
        {connected && !inCall && <button className="btn" onClick={startCall}>Start Call</button>}
        {inCall && <button className="btn alt" onClick={endCall}>End Call</button>}
        <div className="spacer" />
      </div>

      <div className="log glass">
        <div className="log-title">Logs</div>
        <div className="log-body">{logs.map((l,i)=>(<div key={i}>{l}</div>))}</div>
      </div>
    </div>
  )
}
